...
class WSIImage(Dataset):
    def __init__(self, data_feature_path, image_size=256, transform=None):
        ...
        print(f'data_feature_path: {self.data_feature_path}')
...
class Camelyon16Raw(Dataset):
    def __init__(
        ...
        self.labels = pd.read_csv('/data/jlr/FedDFP/data/cam16/labels.csv')
        self.metadata = pd.read_csv('/data/jlr/FedDFP/data/cam16/metadata.csv')
        ...
        for slide in npys_list:
            ...
            slide_pt_pth = f'{self.data_path}/pt_files/{stage}/{slide_pt}'
