...
# import sys
# sys.path.insert(1, '/scratch/sz65/cc0395/FedDDHist/')
import copy
import logging
import math
...
from model.FExtractor import configs
import torch.nn.functional as F
from model.FExtractor.modeling_resnet import ResNetV2
import warnings
    ...
    def __init__(self, config, img_size=224, num_classes=21843, vis=False, args=None):
        ...
        if len(self.share_blocks) > 0:
            self.prompt_common = nn.ParameterList([nn.Parameter(torch.zeros(
                1, 1, config.hidden_size))] + [nn.Parameter(torch.zeros(
        ...
        if self.args.key_prompt > 0:
            self.prompt_embeddings = nn.ModuleDict({str(j): nn.ParameterList([nn.Parameter(torch.zeros(
                1, args.number_prompts, config.hidden_size)) for _ in range(self.num_group)]) for j in range(len(self.share_blocks_g))})
    ...
    def clusters_g(self, x):
        ...
        if self.training:
            ...
            if size_vector.sum() != 0:
                ...
                prompt_sim = (1 - prompt_sim) * ((prob).to(device) + 5e-4)
            else:
...
CONFIGS = {
    'ViT_S_16': configs.get_s16_config(),
...
if __name__ == '__main__':
    model = VisionTransformer_m(CONFIGS['ViT-B_16'], img_size=224, num_classes=1000, vis=False)
