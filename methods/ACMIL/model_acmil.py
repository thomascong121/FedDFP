...
class ACMIL_GA(nn.Module):
    def __init__(self, conf, D=128, droprate=0, n_token=1, n_masked_patch=0, mask_drop=0):
        ...
        self.size = conf.size
        self.dimreduction = DimReduction(self.size[0], self.size[1])
        self.attention = Attention_Gated(self.size[1], D, n_token)
        self.classifier = nn.ModuleList()
        for i in range(n_token):
            self.classifier.append(Classifier_1fc(self.size[1], conf.n_classes, droprate))
        self.n_masked_patch = n_masked_patch
        self.n_token = n_token
        self.Slide_classifier = Classifier_1fc(self.size[1], conf.n_classes, droprate)
        self.mask_drop = mask_drop
